...
export class BillsComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    //get bills to popilate table
    this.transactionService.getBills().subscribe(result => {
      if (result["status"]) {
        console.log(result);
        this.bills = result["success"];
      }
      else {
        console.log(result);
  ...
  viewBill(item) {
    console.log(item);
    window.sessionStorage.setItem("invoiceId", JSON.stringify({id: item.id})); 
  ...
  updateCustomer(customerObject) {
    ...
    customerFormUpdated = this.fbForm.group({
      ...
      pan: [null, [], this.validationService.panValid],
      gst_tin: [null, [], this.validationService.gstValid],
      addressLine1: [customerObject.address1],
      addressLine2: [customerObject.address2],
